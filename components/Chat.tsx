...
const AiChat: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      chatRef.current = ai.chats.create({
        ...
        config: {
          systemInstruction:
            "You are a friendly and helpful cybersecurity assistant for the Cyber Agents Dashboard. Your goal is to answer user questions about online safety, security best practices, and the roles of the agents. Keep your answers concise, informative, and easy to understand. Do not use markdown.",
        },
      });
      setHistory([
        {
          ...
          role: "model",
          content:
            "Hello! I'm the security AI assistant. Ask me anything about cybersecurity or our agents.",
        },
      ]);
    } catch (e) {
      const message =
        e instanceof Error
          ? e.message
          : "Could not initialize the AI chat service.";
      ...
      for await (const chunk of stream) {
        ...
        if (chunkText) {
          ...
          setStreamingMessage((prev) =>
            prev ? { ...prev, content: finalContent } : null,
          );
        }
      }
    } catch (err) {
      ...
      const finalMessage: ChatMessage = {
        ...
        content:
          ...
          (streamError
            ...
            : "Sorry, I seem to be having trouble connecting."),
      };
  ...
  const renderMessage = (msg: ChatMessage) => (
    <div
      key={msg.id}
      className={`flex items-start gap-3 max-w-xl ${msg.role === "user" ? "ml-auto flex-row-reverse" : ""}`}
    >
      <div
        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === "user" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-600"}`}
      >
        {msg.role === "model" ? <Bot size={20} /> : <User size={20} />}
      </div>
      <div
        className={`rounded-lg px-4 py-2 text-gray-800 ${msg.role === "user" ? "bg-blue-100" : "bg-gray-100"}`}
      >
        {msg.streaming && msg.content === "" ? (
          <div className="flex items-center h-6">
            <span
              className="w-2 h-2 bg-blue-500 rounded-full animate-pulse mr-1.5"
